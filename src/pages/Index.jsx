...
const ProductCard = ({ product, addToCart }) => (
  <Box borderWidth="1px" borderRadius="lg" overflow="hidden" p="5" textAlign="center" position="relative">
    ...
    <Flex mt="3" justifyContent="center" alignItems="center">
      ...
      <Button leftIcon={<FaShoppingCart />} colorScheme="teal" variant="solid" size="sm" onClick={() => addToCart(product.id)}>
        Add to Cart
      </Button>
    </Flex>
  </Box>
...
const Index = () => {
  ...
  const addToCart = (productId) => {
    setCart((currentCart) => {
      const isProductInCart = currentCart.find((item) => item.id === productId);
      if (isProductInCart) {
